...
### 2. 프레임 추출 🤖
```bash
python 01_extract_frames.py --name lecture_book --fps 2
```

### 3. SAM3 자동 라벨링 🤖
```bash
python 02_label_sam3.py --name lecture_book --prompt "book"
# 같은 이름이 있으면 lecture_book_v2 로 자동 생성
```

### 4. QC 자동 선별 🤖
```bash
python 03_qc_flag.py --name lecture_book
...
### 6. 데이터셋 확정 🤖
```bash
python 05_export_yolo.py --name lecture_book
# 수정본을 자동 병합(JSON→YOLO) + data.yaml 생성 (train=train, val=test)
```

### 7. 모델 선택 & 학습 👤+🤖
```bash
python 06_train_yolo.py --name lecture_book
...
### 8. 탐지 / 실시간 카메라 확인 🤖
```bash
python 07_predict.py --name lecture_book --source camera      # 웹캠 실시간
python 07_predict.py --name lecture_book --source 새영상.mp4   # 영상 파일
```
...
- **수정 후 흐름**: labelme로 고친 뒤 `05_export_yolo.py` 실행만 하면 **자동 병합** (SAM3 재실행·수동 이동 불필요)
