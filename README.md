...
> 🔁 **한 번 더 검수하고 싶다면 4번(`03_qc_flag`)을 다시 실행하세요.** 직전 검수로 라벨이
> 개선됐으면 재평가 시 보통 더 적게 flagged 됩니다. `03 → labelme 수정 → 03 다시` 를
> 의심분이 충분히 줄 때까지 반복한 뒤, 만족스러우면 아래 6번으로 넘어갑니다.
> ```bash
> python scripts/03_qc_flag.py --name lecture_book   # _needs_review/ 재생성 → 다시 labelme
> ```
...
> 👀 **확정 전에 라벨 전체를 한 번 더 눈으로 확인하세요.** 아래 [라벨 전체 눈으로 확인하기](#-라벨-전체-눈으로-확인하기-보조-도구)
> 의 `visualize_labels.py` 로 박스를 그려 `_viz/` 에 뽑은 뒤 훑어보고, 이상한 게 보이면
> 5번(labelme)·4번(`03_qc_flag`)으로 돌아가 다시 손봅니다.
> ```bash
